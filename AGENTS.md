...
- **2026-07-19**: Added a black “Turn up your audio” reminder immediately after Enter the Museum and before the narrated opening in `src/game.js` and `styles.css`; refreshed unified runtime versions in `index.html`, documented the entry beat in `README.md`, and added timing, styling, and release-route coverage in `tests/browser_smoke.py` and `tests/release_qa.py`.
- **2026-07-19**: Completed the automated final-check matrix across `tests/release_qa.py`, `scripts/verify.sh`, `src/game.js`, `styles.css`, `index.html`, `README.md`, `assets/generated/README.md`, and `TODO.md`; verified both investigators and all production frames, every collision obstacle, keyboard/focus navigation, accessibility semantics, reduced motion, large text, five effective viewport/zoom sizes, a fresh-state true-ending route, and clean console/network behavior; replaced the deleted viewer-background request that caused 404s, restored reliable Space activation on focused controls, corrected foreground depth overlap, and passed the complete verifier from a separate clean checkout.
